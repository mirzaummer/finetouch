            ...
            foreach ($this->helper('payment')->getStoreMethods($store, $quote) as $method) {
                if ($this->_canUseMethod($method) && $method->isApplicableToQuote(
                    ...
                    if($method->getCode() == "purchaseorder")
                        Mage::getSingleton('core/session')->setMyValue('no');
                }
                else if($method->getCode() == "purchaseorder")
                {
                    $this->_assignMethod($method);
                    $methods[] = $method;
                    Mage::log('gaaaaaaaaaaaaaaaaaaa2');
                    Mage::getSingleton('core/session')->setMyValue('yes');
                }
            }
